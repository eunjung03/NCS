...
public class Car_main {
	...
	// JVM이 main() 메서드를 호출하는 데 미리 메모리에 로딩되어 있지 않다면
	// 인스턴스(객체)를 생성을 하고 main() 메서드를 호출해야 하므로, 
	// 기본적으로 main() 메서드를 자동으로 호출하기 위해서 static을 붙인 것임.
	
	public static void main(String[] args) {
		
		System.out.println("차량 종류 >>> " + CarDemo.kind);
		
		CarDemo myCar = new CarDemo();	// 인스턴스 객체 생성
		...
		myCar.speedDown();		// speed는 한 번 호출할 때마다 5씩 감소
		myCar.speedDown();
		System.out.println("myCar의 스피드 >>> "+myCar.speed);
