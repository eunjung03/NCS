 ...
 *    			- protected : 같은 패키지 접근 가능. 다른 패키지는 상속관계는 접근 가능.
 ...
 *             - 반환형이 void 인 경우를 제외하고 결과를 되돌려주어야 할 때는 메서드 명령문
 *               맨 마지막에 return 이라는 키워드를 사용 후, 앞에서 선언한 반환형과 같은 
 *               자료형으로 결과를 되돌려준다.
 *    - 메서드 이름 : - 식별자, 소문자로 시작, 두 개의 단어가 결합된 경우에는 두번째 글자의 첫 글자를
 *      		         대문자로 작성하는 것이 관례임.
