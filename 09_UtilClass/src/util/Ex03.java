 ...
 *    - 추상클래스로 작성된 이유 : 날짜와 시간을 계산하는 방법이 지역, 문화, 나라마다 다르기 때문.
...
public class Ex03 {
	...
	public static void main(String[] args) {
		...
		int week = cal.get(Calendar.WEEK_OF_YEAR);	// 1년 52주 중 몇번째 주인지 확인
		...
		System.out.println(am_pm);		// 오전 or 오후인지 확인하는 방법
		
		if(am_pm == 0) {
			// 오전인 경우
			System.out.println("현재 시간은 AM(오전) "+hour+"시 "+minute+"분 "+second+"초 입니다.");
		} else {
			// 오후인 경우
			System.out.println("현재 시간은 PM(오후) "+hour+"시 "+minute+"분 "+second+"초 입니다.");
		}
		System.out.println();
