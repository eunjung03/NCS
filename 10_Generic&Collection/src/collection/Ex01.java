 ...
 *      => ArrayList(0), LinkedList(X), Vector(X)
 *    - 특히 DB에 저장된 데이터를 레코드 단위로 저장하거나 가져올 경우 많이 사용이 됨.
 */
...
public class Ex01 {
	...
	public static void main(String[] args) {
		...
		// 배열 객체 생성
		String[] str = {"홍길동", "이순신", "유관순", "홍길동"};		// 중복 데이터 허용 : 배열엔 index가 있기 때문에 구분 가능.
		...
		// 5. list의 특정 요소(index)를 삭제하는 메서드 : remove(index)
		...
		// 해당 index로 추가가 되고, 기존에 있던 데이터들은 뒤로 한 index씩 밀려난다.
		list.add(2, "세종대왕");
		for(int i=0; i<list.size(); i++) {
			System.out.println("list["+i+"] >>> " + list.get(i));
		}
