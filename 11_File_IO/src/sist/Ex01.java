 ...
 * - 단방향성 : 한쪽 방향으로만 입력되고 출력이 됨.
 * - FIFO(First In First Out) : 먼저 입력되어 먼저 출력됨.                   
 */
...
public class Ex01 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// read() : 표준입력장치를 통해서 입력받은 데이터를 읽음.
			//          읽으면 ASCII 코드(숫자)로 반환.
			int num = System.in.read();
			System.out.println("읽어온 데이터(ASCII) >>> "+num);
			System.out.println("읽어온 데이터(char) >>> "+(char)num);
		} catch (IOException e) {
			e.printStackTrace();
		}
