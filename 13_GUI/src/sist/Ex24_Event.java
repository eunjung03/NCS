 ...
 * - 현재 발생한 이벤트에 관한 정보를 가진 객체를 의미함. 이벤트 리스너에게 전달됨.
 *   => 예를 든다면 버튼을 눌렸다던지, 리스트 메뉴를 선택하였을 때 등등
 ...
 * - 컴포넌트와 리스너를 연결시키는 메서드 : addXXXXListener() ==> XXXX는 리스너의 이름
 * - 이벤트 리스너 작성 과정.
 ...
 *    2) 버튼 클릭은 운영체제의 마우스 드라이버를 거쳐서 자바 가상기계(JVM)에 전달됨.
 *    3) 자바가상기계는 이벤트 분배 스레드에게 마우스 클릭에 관한 정보를 전달함.
 *    4) 이벤트 분배 스레드는 이벤트(ActionEvent) 객체를 생성함. 
 *       이벤트 객체는 이벤트에 관련된 여러 정보를 담고 있는 객체임. 
 *       이벤트 객체 내에 저장되는 정보 중에 특별히 이벤트를 발생시킨 컴포넌트를 이벤트 소스라고 부름.
 *       버튼에 글자가 새겨진 JButton 컴포넌트임.
 *    5) 이벤트 분배 스레드는 JButton에 연결된 버튼 컴포넌트와 관련된 이벤트 리스너를 찾아서 호출함.
 *    6) 이벤트 분배 스레드는 다음 이벤트를 기다림.
 *    
 * [이벤트 관련 용어]
 *  - 이벤트 소스 : 이벤트를 발생시킨 GUI 컴포넌트를 말함.
 *  - 이벤트 객체 : 발생한 이벤트에 대한 정보(이벤트 종류, 이벤트 소스, 화면 좌표, 마우스 버튼의 종류, 눌려진 키)를 담는 객체로서, 
 * 				 이벤트에 따라 서로 다른 정보가 저장됨.
 *  - 이벤트 리스너(Event Listener) : - 이벤트를 처리하는 코드로서 컴포넌트에 등록이 되어야 작동이 가능함.
 * 					               - 이벤트 소스와 이벤트 핸들러를 연결.
 *  - 이벤트 분배 스레드 : 이벤트 기반 프로그래밍의 핵심 요소로서 무한 루프를 실행하는 스레드임.
 * 					   자바 가상 기계로부터 이벤트 발생의 통지를 받아서, 이벤트 소스와 이벤트 종류를 결정하고
 * 					   이에 따라 적절한 이벤트 객체를 생성하고 이벤트 리스너를 찾아 호출함.
 */
